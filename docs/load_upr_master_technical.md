...
2. Dedupe to `#IncomingUnique` — one row per property key:
   - **CONDO / MULTI / APT:** one row per Account# (all occurrences share the property)
   - **Other types:** one row per account + normalized address
3. Non-multi extras → Review_Q `DUPLICATE`; multi-unit extras → `dbo.Unit` (not DUPLICATE)
4. From `#IncomingUnique`: valid → UPR; invalid → Review_Q
5. MA/SDAT mismatch: use valid address / preferred 8-digit account for UPR candidate; still flag Review_Q
6. External systems match UPR by address only → XREF
...
| CONDO / Multi-Family / APT | Same Account# → **one UPR** + **all occurrences in `dbo.Unit`**; not Review_Q `DUPLICATE` (unless other invalid data) |
| MA/SDAT address mismatch | Prefer valid address side for UPR; still Review_Q `Address or Account Not Match` |
| MA/SDAT account mismatch | Prefer 8-digit account for UPR; still Review_Q `Address or Account Not Match` |
...
| `fn_UPR_IsPreferredAccount` | True when normalized account is exactly 8 digits (preferred on mismatch) |
| `fn_UPR_AddressQualityScore` | 0–4 score (street #, name, city, zip) for MA vs SDAT mismatch reconciliation |
...
## Section 10 — Building + Unit (CONDO / MULTI / APT)

For UPR rows where `REF_PROPERTYTYPE.AllowsBuildings = 1` and `AllowsUnits = 1`:

- Insert `Building` code `MAIN` if missing
- Insert **all** `#IncomingUnified` multi-unit occurrences (same Account#) as `Unit` rows
- `UnitNumber` = MA `Unit` when present; otherwise `U` + occurrence rank
- Deduped on `(UPropertyRecordsID, UnitNumber)`
- Multi-unit extras are **not** Review_Q `DUPLICATE`
