...
================================================================================
  ...
  DHCA SOURCE DATABASES (client standard — edit names below if needed):
    DHCA_Internal, DHCA_LicensingAndRegistration, DHCA_OLTA, DHCA_MPDU, DHCA_MultifamilyLoans
  Cross-database reads use dynamic SQL (no synonym setup, no SSMS red underlines).
================================================================================
*/
USE UPRDB_Test;
GO
...
DECLARE @MasterAddressRead INT = 0, @SDATRead INT = 0, @IncomingUnifiedRows INT = 0, @BadIncomingRows INT = 0;
DECLARE @UPRInserted INT = 0, @UPRExistingMatched INT = 0;
...
/* DHCA source database names (client standard) */
DECLARE @DbInternal    SYSNAME = N'DHCA_Internal';
DECLARE @DbLicensing   SYSNAME = N'DHCA_LicensingAndRegistration';
DECLARE @DbOLTA        SYSNAME = N'DHCA_OLTA';
DECLARE @DbMPDU        SYSNAME = N'DHCA_MPDU';
DECLARE @DbMultifamily SYSNAME = N'DHCA_MultifamilyLoans';
...
BEGIN TRY
    ...
    SELECT
        MasterAddressID, Account, ParcelNumber, StreetNumber, StreetName, StreetType,
        Unit, City, ZipCode, LUCategory, XCoordinate, YCoordinate
    FROM ' + QUOTENAME(@DbInternal) + N'.dbo.MasterAddress';
    ...
    SELECT
        RealPropertyTaxInformationID, AccountNumber, Parcel, PremisesNumber, PremisesStreetName,
        PremisesStreetType, PremisesCity, PremisesState, PremisesZipCode, Owner, YearBuilt, DwellingUnits
    FROM ' + QUOTENAME(@DbInternal) + N'.dbo.RealPropertyTaxInformation';
    EXEC sys.sp_executesql @Sql;
    ...
    SELECT
        ...
        EffectiveNormalizedStreetAddress = LEFT(NULLIF(LTRIM(RTRIM(w.NormalizedStreetAddress)), N''), 100),
        EffectiveNormalizedFulldAddress  = LEFT(NULLIF(LTRIM(RTRIM(w.NormalizedFullAddress)), N''), 100),
        ...
        IsEligibleForUpr = CASE
            WHEN NULLIF(LTRIM(RTRIM(w.ParcelID)), N'') IS NULL THEN 0
        ...
        AddressRn = ROW_NUMBER() OVER (
            ...
            ORDER BY CASE w.MatchSource WHEN N'BOTH' THEN 1 WHEN N'KDAT' THEN 2 ELSE 3 END,
                     COALESCE(w.SDATAccountNumber, w.ParcelID, N'')
        ),
        ParcelRn = ROW_NUMBER() OVER (
            PARTITION BY NULLIF(LTRIM(RTRIM(w.ParcelID)), N'')
            ORDER BY CASE w.MatchSource WHEN N'BOTH' THEN 1 WHEN N'KDAT' THEN 2 ELSE 3 END,
                     COALESCE(w.SDATAccountNumber, w.ParcelID, N'')
        )
    INTO #UprCandidate
    ...
    /* Stage bad/duplicate incoming rows for review — specific rejection reasons */
    INSERT INTO #ReviewPending (
        MasterAddressID, KdatRecordID, MatchSource, NormalizedIncomingAddress,
        ParcelID, SDATAccountNumber, ReasonForNoMatch,
    ...
    SELECT
        ...
        CASE
            WHEN NULLIF(UPPER(LTRIM(RTRIM(w.StreetName))), N'') IS NULL THEN N'MISSING_STREET_NAME'
            WHEN NULLIF(UPPER(LTRIM(RTRIM(w.City))), N'') IS NULL THEN N'MISSING_CITY'
            WHEN dbo.fn_UPR_IsValidStreetNumber(w.StreetNumber) = 0 THEN N'INVALID_STREET_NUMBER'
            WHEN dbo.fn_UPR_IsValidZipCode(w.ZipCode) = 0 THEN N'INVALID_ZIP_CODE'
            WHEN NULLIF(LTRIM(RTRIM(w.ParcelID)), N'') IS NULL THEN N'MISSING_PARCEL_ID'
            ELSE N'INSUFFICIENT_ADDRESS_DATA'
        END,
    ...
    WHEN MATCHED THEN UPDATE SET
        tgt.ParcelID          = COALESCE(
            ...
            tgt.ParcelID,
            src.EffectiveParcelID
        ),
        tgt.Owner             = LEFT(COALESCE(tgt.Owner, src.EffectiveOwnerName), 100),
        tgt.Latitude          = COALESCE(tgt.Latitude, src.EffectiveLatitude),
        tgt.Longitude         = COALESCE(tgt.Longitude, src.EffectiveLongitude),
    ...
    WHEN NOT MATCHED THEN INSERT (
        ...
        src.EffectiveSDATAccountNumber, src.EffectiveParcelID, NULL, src.EffectiveOwnerName,
    ...
    CROSS APPLY (
        ...
        WHERE upr.SDATAccountNumber = c.EffectiveSDATAccountNumber
           ...
           OR upr.ParcelID = c.EffectiveParcelID
           OR (upr.NormalizedFulldAddress = c.EffectiveNormalizedFulldAddress)
           OR (
                upr.StreetNumber = c.EffectiveStreetNumber
                AND upr.StreetName = c.EffectiveStreetName
                AND upr.StreetType = c.EffectiveStreetType
                AND upr.ZipCode = c.EffectiveZipCode
           )
        ORDER BY
            CASE WHEN upr.SDATAccountNumber = c.EffectiveSDATAccountNumber THEN 1
    ...
    FROM ' + QUOTENAME(@DbLicensing) + N'.dbo.Property ep';
    EXEC sys.sp_executesql @Sql;
    ...
    FROM ' + QUOTENAME(@DbOLTA) + N'.dbo.[Case] c';
    EXEC sys.sp_executesql @Sql;
    ...
    FROM ' + QUOTENAME(@DbMPDU) + N'.dbo.Development mp';
    EXEC sys.sp_executesql @Sql;
    ...
    FROM ' + QUOTENAME(@DbMultifamily) + N'.dbo.Address mf
    ...
    /* eProperty — NOT EXISTS on SourceSystem + SourceRecordID + EntityType + DeleteInd */
    INSERT INTO dbo.UPROPERTYRECORDS_XREF (
        UPropertyRecordsID, SourceSystemCode, SourceRecordID, SourceEntityType,
        MatchMethodCode, MatchResult, MatchConfidence, ProcessingStatus,
        IsActive, EffectiveStartDate, Notes, CreatedDate, UpdatedDate, CreatedBy
    ...
    /* ========================================================================
       8. REVIEW QUEUE — properties with no match in any external system
       ======================================================================== */
    INSERT INTO dbo.UPRMATCHREVIEW_Q (
        UPropertyRecords_XrefID, IncomingSourceSystem, NormalizedIncomingAddress,
        ParcelID, SDATAccountNumber, ReasonForNoMatch, ReviewStatus
        ...
        WHERE x.UPropertyRecordsID = m.UPropertyRecordsID
          ...
          AND (
                (m.KdatRecordID IS NOT NULL
                 ...
                 AND x.SourceRecordID = CONVERT(NVARCHAR(100), m.KdatRecordID))
             OR (m.MasterAddressID IS NOT NULL
                 AND x.SourceSystemCode = N'ADDRESS_MASTER'
                 AND x.SourceRecordID = CONVERT(NVARCHAR(100), m.MasterAddressID))
          )
    ...
    WHERE m.HasRequiredAddress = 1
      ...
      AND NOT EXISTS (
          ...
          WHERE q.UPropertyRecords_XrefID = xref.UPropertyRecords_XrefID
            AND q.ReasonForNoMatch = N'NO_EXTERNAL_MATCH'
      );

    SET @ReviewExternalInserted = @@ROWCOUNT;
    SET @ReviewInserted = @ReviewIncomingInserted + @ReviewExternalInserted;
    ...
    PRINT N'Unified incoming rows prepared: ' + CONVERT(VARCHAR(20), @IncomingUnifiedRows);
    PRINT N'Bad/incomplete incoming rows: ' + CONVERT(VARCHAR(20), @BadIncomingRows);
    PRINT N' ';
    PRINT N'UPropertyRecord inserted: ' + CONVERT(VARCHAR(20), @UPRInserted);
    PRINT N'Existing UPR matched/skipped dup: ' + CONVERT(VARCHAR(20), @UPRExistingMatched);
    PRINT N' ';
    ...
    PRINT N'  (incoming rejected: ' + CONVERT(VARCHAR(20), @ReviewIncomingInserted)
        + N', no external match: ' + CONVERT(VARCHAR(20), @ReviewExternalInserted) + N')';
