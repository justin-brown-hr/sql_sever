...
PRINT N'Duplicate groups with valid rows but no UPR on account: ' + CONVERT(NVARCHAR(20), @MisRoutedGroups);

IF @MisRoutedGroups > 0
    PRINT N'';
    PRINT N'>>> CHECK Section 2: one row per group should be UPR winner; others are DUPLICATE losers.';
ELSE
    PRINT N'';
    PRINT N'No duplicate groups missing UPR on account (expected after incoming-unified-dedup fix).';
